...
$certificate = Get-ChildItem -Path Cert:\CurrentUser\My | Where-Object { $_.Thumbprint -eq $config.CertificateThumb }

if (-not $certificate) {
    Write-Error "❌ Certificate with thumbprint $($config.CertificateThumb) not found in CurrentUser\My store."
    exit 1
}
